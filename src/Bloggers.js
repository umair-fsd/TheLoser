...
class Artical extends Component {
  ...
  forceUpdateHandlerUSERlikes() {
    ...
    axios.post(window.$API_URL + "tbl_blogges_likes", obj).then((res) => {
  ...
  forceUpdateHandleralllikes() {
    ...
    axios.post(window.$API_URL + "Bloggsalllikes", obj).then((res) => {
      const formatCash = (n) => {
        if (n < 1e3) return n;
  ...
  forceUpdateHandler() {
    ...
    axios
      .post(window.$API_URL + "bloggsfulscreen", this.state.id)
      .then((res) => {
        console.log(res.data, "article");
        this.setState({
          count: res.data[0].description,
          componentupdate: false,
          username: res.data[0].username,
          imagename: res.data[0].image,
          // category: res.data[0].category,
          title: res.data[0].title,
          // sub_category: res.data[0].sub_category,
          user_img: res.data[0].user_img,
          video_id: res.data[0].bloggers_id,
  ...
  forceUpdateHandlerVideos() {
    ...
    //   }
    axios.post(window.$API_URL + "forceUpdateHandlerbloggs").then((res) => {
      var grid = [];
  ...
  like() {
    ...
    if (data === null) {
      ...
      axios.post(window.$API_URL + "blogslike", obj).then((res) => {
        if (res.data === 1) {
          this.setState({
            like: "1",
          });
  ...
  unlike() {
    ...
    if (data === null) {
      ...
      axios.post(window.$API_URL + "bloggsUnlike", obj).then((res) => {
        if (res.data === 1) {
          this.setState({
            like: "",
          });
