...
var user_id = "1";
class Articalslist extends React.Component {
  componentWillMount() {
  ...
  fetchdata() {
    ...
    axios
      .post(window.$API_URL + "articlesgallery", this.state.articlelist)
      .then((res) => {
  ...
  Mostviewed() {
    axios
      .post(
        window.$API_URL + "articlesgalleryMostviewed",
  ...
  Recentuploaded() {
    axios
      .post(window.$API_URL + "gallerysRecentuploaded", this.state.articlelist)
      .then((res) => {
        this.setState({
          articlelist: this.props.match.params.value,
        });
